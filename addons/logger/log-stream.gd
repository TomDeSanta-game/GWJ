...
func log_filename()->String:
	var dir = "user://logs/"
	var now = Time.get_datetime_dict_from_system(false)
	var fname = dir+"godot_log_{year}-{month}-{day}-{hour}-{minute}-{second}.log".format(now)
	
	return fname

func log_to_file(ensure_exists:=true)->FileAccess:
	var fname = log_filename()
	if !_log_file:
		var dir = DirAccess.open("user://")
		if !dir.dir_exists("logs") && ensure_exists:
			dir.make_dir("logs")
		
		if ensure_exists:#Make sure the file exist.
			_log_file = FileAccess.open(fname, FileAccess.WRITE)
			_log_file.close()
		_log_file = FileAccess.open(fname, FileAccess.READ_WRITE)
	return _log_file
